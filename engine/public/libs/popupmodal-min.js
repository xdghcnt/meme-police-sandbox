...
(function () {
    ...
    var hideModal = function hideModal() {
        ...
        setTimeout(function () {
            ...
            setTimeout(function () {
                const curr_modal_node = document.getElementById(curr_modal);
                if (curr_modal_node)
                    document.body.removeChild(curr_modal_node);
                ...
                if (modal_created < 1) {
                    const curr_backdrop_node = document.getElementById(curr_backdrop);
                    if (curr_backdrop_node)
                        document.body.removeChild(curr_backdrop_node);
                } else {
                    document.getElementById(curr_backdrop).style.background = config_arr[config_arr.length - 1].bg_overlay_color;
                    document.getElementById(curr_backdrop).style.zIndex = modal_created * 10;
    ...
    var modal = function modal(type, options, callback, isTextarea) {
        var input_len = !isTextarea ? 99 : 3000,
            placeholder = "",
            value = "",
            ...
            if (options && (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object') {
                ...
                if (options.value) {
                    ...
                    if (typeof options.value === 'string') {
                        ...
                        throw 'value is not type string';
                    }
                }
        ...
        if (type === 'prompt') {
            var modal_input = document.createElement(!isTextarea ? 'input' : 'textarea');
            if (!isTextarea)
            ...
            if (isTextarea)
                modal_input.addEventListener('keydown', (e) => e.keyCode === 13 && e.stopPropagation())
    ...
    var textarea = function textarea(options, callback) {
        modal('prompt', options, callback, true);
    };
